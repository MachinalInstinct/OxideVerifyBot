...
{
    [Info("DiscordLink", "MachinalInstinct", "1.0.1")]
    internal class MysqlTest : RustPlugin
    {
        ...
        private void LinkQuery(BasePlayer player, string command, string[] args)
        {
            ...
            _mySqlConnection = _mySql.OpenDb("mysql.server.ip", 3306, "mysql.server.db", "mysql.server.duser", "mysql.server.password", this);
            ...
            {
				if (list.Count <= 0){ player.ChatMessage(AuthKeyWrongMessage); }
                ...
                if (list == null) return;
					...
					else { player.ChatMessage(AlreadyAuthMessage); }
					}			
				}				
